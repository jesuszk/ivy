...
class SalesAlignmentRequest extends Request
{
    ...
    function toEtapaVendasAlinhamento(): array
    {
        ...
        $data = $this->get();
        return (array_only($data, $columns));
    }
    ...
    function toEtapaVendasAlinhamentoItens(): array
    {
        ...
        foreach ($_POST["block_item"] as $idx => $item) {
            $itens[] = array_merge($item, ["processo_uuid" => $this->get("processo_uuid"), "etapa_vendas_alinhamento_uuid" => $this->get("etapa_vendas_alinhamento_uuid")]);
        }
        return $itens;
    }
}
