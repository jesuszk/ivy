<?php

namespace src\requests;

use src\support\Request as httpRequest;

abstract class Request
{

    function __construct()
    {
        $this->execute();
    }

    /** @var array<string, string> */
    protected array $rules;

    protected bool $ignoreRules;

    /**
     * This method returns all inputs except the token
     * 
     * @param ?string $key
     * @return array<mixed, mixed>|string
     */
    public function get(?string $key = null): array|string
    {
        if (!$key)
            return httpRequest::excepts(["token"]);
        return $this->input($key);
    }


    /**
     * This method returns an index of Superglobal $ _Post
     * @param string $input
     * @return string|array<mixed, mixed>
     */
    public function input(string $input): string|array
    {
        return httpRequest::input($input);
    }

    /**
     * This method is responsible for returning the validation rules
     * 
     * @return array<string, string>
     */
    public function rules(): array
    {
        return $this->rules;
    }

    /**
     * Method responsible for performing validation
     * @return array<void>
     */
    public function execute(): array
    {
        if (!$this->ignoreRules)
            if (!formValidate($this->rules())) {
                notification()->error('Verifique todos os campos antes de prosseguir');
                redirect()->back()->make();
                die;
            }
        return [];
    }



    function toProcesso(string $currentChapter, ?string $previousChapter = null)
    {
        $additional = [
            "uuid" => $this->get("processo_uuid"),
            "estado" => $_ENV["STATE_IN_PROGRESS"],
            "etapa_atual" => $currentChapter,
            "etapa_anterior" => $previousChapter
        ];


        return $additional;
    }


    function toObservacao()
    {
        return [
            "processo_uuid" => $this->get("processo_uuid"),
            "etapa" => $this->get("etapa_atual"),
            "usuario_chave" => $_SESSION["usuarioLogin"],
            "usuario_nome" => $_SESSION["usuarioNome"],
            "descricao" => $this->get("observacoes")
        ];
    }

    function log()
    {
        make_log($this->get("log"), $this->get("processo_uuid"), $this->get("etapa_atual"), $_SESSION["usuarioLogin"],  $_SESSION["usuarioNome"]);
    }
}
