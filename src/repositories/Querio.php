...
class Querio
{
    ...
    {
        if (!str_contains($this->queryString, "UPDATE"))
            $this->bind = [];
     ...
     */
    function finish($lvl = 0): object|array|bool
    {
        try {
    ...
    function create(array $data): bool|array
    {
        ...
        return $this->table($this->table)->insert($data)->finish(1);
    }
    ...
    function getByColumnMany(string $column, string $value, string $operation = "="): array|bool
    {
        return $this->table($this->table)->select()->where($column, $operation, $value)->finish();
    }
     ...
     * @return bool
     */
    function deleteByColumn(string $column, mixed $value): bool
    {
        return $this->table($this->table)->delete()->where($column, "=", $value)->finish();
    }
    ...
    {
        return $this->table($this->table)->update($data)->where("uuid", "=", $uuid)->finish(1);
    }

    function updateByColumn(string $column, array $data)
    {
        return $this->table($this->table)->update($data)->where($column, "=", $data[$column])->finish(1);
    }
