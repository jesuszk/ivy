...
class ViewEditCommand extends Command
{
    protected function configure()
    {
        $this
            ->setName('view:edit')
            ->setDescription('Create a new edit view file')
    ...
    },
    {
        ...
        $content = $this->generateEditContent(
            $name,
            $plural,
            $fields
        );
        ...
        $output->writeln("<info>Edit view created successfully at {$filename}</info>");
        return Command::SUCCESS;
    }

    private function generateEditContent(
        string $name,
        string $plural,
        array $fields
    ): string {
        ...
        $layoutParams = [
            ...
            'cardTitle' => 'Edit ' . ucfirst($name)
        ];

        $content = "<?= \$this->layout('templates/base', " . $this->arrayToPhpString($layoutParams) . ") ?>\n\n";

        // Generate form
        $content .= "<form action=\"<?= route('{$plural}.update', ['uuid' => \${$name}->uuid]); ?>\" method=\"post\">\n";
        $content .= "    <div class=\"row g-3\">\n";

        // Add hidden ID field
        $content .= "        <input type=\"hidden\" name=\"uuid\" value=\"<?= \${$name}->uuid; ?>\">\n\n";
        ...
        foreach ($fields as $field) {
            ...
            if ($fieldType === 'select') {
                ...
                foreach ($field['options'] as $option) {
                    $selected = "<?= \${$name}->{$fieldName} == '{$option['value']}' ? 'selected' : ''; ?>";
                ...
                $content .= "            <input type=\"{$inputType}\" class=\"form-control\" name=\"{$fieldName}\" id=\"{$fieldName}\" value=\"{$value}\"";
                if (isset($field['step'])) {
        ...
        $content .= "            <button class=\"btn btn-company float-end\">Update <i class=\"ph ph-paper-plane-tilt\"></i></button>\n";
        $content .= "        </div>\n";
        $content .= "    </div>\n";
        $content .= "</form>\n";
